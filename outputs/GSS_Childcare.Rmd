...
subtitle: "An Analysis on the General Social Survey: Family, Cycle 31, 2017"
abstract: "This paper studies the usage, type, reasoning, finance, cost of Childcare Services in Canada amongst the respondents in the General Social Survey: Family, Cycle 31, 2017. Respondents’ education, main activity before and after parental leave, and the average costs of Childcare arrangements are explored. In this report, R [@citeR] is used to visualize the data and help perform reliable statistical analysis about the aspects focused on in the discussion. Eventually, this report can be used to evaluate the current early child development services in Canada, and a supplementary survey is provided to gather more data for further analysis."
author: 
...
In Canada, there are several programs designed for the purpose of caring for children in the temporary absence of their parents such as day nurseries, daycare child care, foster home care and family home daycare and [@howe_prochner]. These comprise the enterprise of early childhood education. The history of childcare services and education is that most of them were privately sponsored programs, organized and headed by women, and the debate over the role of parents versus the private charity providing care and education for young children  [@howe_prochner]. Earlier, childcare was provided to poor families as a social service in times of critical need [@howe_prochner]. 
This paper focuses on the Childcare aspect of the General Social Survey (GSS), Cycle 31, 2017 microdata, which is retrieved from the University of Toronto CHASS (Computing in Humanities and Social Sciences) [@technology]. The aim of this paper is to understand the Canadian Childcare service usage, type of childcare uses, main reasons to get childcare, costs of childcare, and how it relates to socioeconomic factors in the microdata provided, focusing on financial support and education. In general, parents were primarily responsible for the early education of their children but this was up for debate in kindergartens in public schools. There are still no national plans directing childcare policy all across Canada in provinces and territories and kindergartens continue to have an ambiguous place in schools [@howe_prochner].

These different aspects of Childcare enable us to not only look at child care as a social policy issue but also a multidimensional issue, beyond the simplification of the mother’s care vs non-family care dichotomy [@saraceno_2011]. This paper gives an overview of the usage, type, and costs of childcare service usage in Canada and on the labor market activity before parental leaves and parental education which is necessary as they shed light on and help develop policies that address the complex issues of choice, rights, and social inequality involved in child caring patterns[@saraceno_2011].
...
## Results
It is important to understand the dynamics of childcare. To explore further into this, it is essential to understand the basic usage of childcare arrangements amongst the respondents. Figure \@ref(fig:ccbasis), created using the @citeggplot function tells us about the usage of childcare. The majority of our respondents, approximately 1250, use childcare on a regular basis, and around 600 approximately, use it on an occasional or irregular basis. It is essential to understand that Figure \@ref(fig:ccbasis) doesn't account for education, income, and other socio-economic factors and solely focuses on the usage of childcare. In addition, Figure \@ref(fig:ccusage) also illustrates how often childcare was used in the past month. According to the respondents, most of them use childcare five days a week. The number of people who use childcare services five days a week greatly exceeds the other 7 options, which highlights the importance of childcare services during business days.




```{r ccbasis, fig.cap="Child-Care Usage Basis Amongst Respondents",fig.pos="H", echo = FALSE}
...
theme_bw <- 
  ...
  ggplot(data = subset(data,!is.na(cc_basis)),  aes(x = cc_basis, y = frequency(cc_basis), main="Child-Care Usage", fill = cc_basis)) + labs(fill= "Childcare usage basis") +
         ...
         coord_flip() + 
         ggtitle("Child-Care Usage Basis Amongst Respondents") +
         scale_y_continuous(name="Count") +
         scale_x_discrete(name="Child-Care Usage among Respondents") +
         theme_bw()
```

```{r ccusage, fig.cap="How Often was Child Care used in the Past Month",fig.pos="H", echo = FALSE}
theme_bw <-
  options(repr.plot.width=15, repr.plot.height=6)
  ggplot(data = subset(data,!is.na(cc_usage)),  aes(x = cc_usage, y = frequency(cc_usage), main="Child-Care Usage", fill = cc_usage)) + labs(fill = "Days per Week") +
         geom_bar(stat = "identity") +
         coord_flip() +
         ggtitle("How Often was Child Care used in the Past Month") +
         scale_y_continuous(name="Count") +
         scale_x_discrete(name="Days of Using Child Care in a Week") + ggtitle("How Often was Child Care used in the Past Month") +
    theme_bw()
    
```

```{r ccfin, fig.cap="Financial Support Amongst Respondents" , fig.pos="H", echo = FALSE}
#Financial Support
...
theme_bw <- 
  ...
  ggplot(data = subset(data,!is.na(ever_fin_sup)),  aes(x = ever_fin_sup, y = frequency(ever_fin_sup), main="Child-Care Usage", fill = ever_fin_sup)) + labs(fill= "Type of Financial Support") +
         geom_bar(stat = "identity") + 
         coord_flip() + 
         ggtitle("Financial Support amongst Respondents") +
         scale_y_continuous(name="Count") +
...
```{r ccfinbasis, fig.cap="Financial Support and Usage of childcare" , echo = FALSE, out.width="300%"}
...
theme_bw <- 
  ...
  ggplot(data = subset(data,(!is.na(ever_fin_sup)) & (!is.na(cc_basis))),  aes(x = ever_fin_sup, y = frequency(ever_fin_sup), main="Child-Care Usage", fill = cc_basis)) +labs(fill= "Childcare usage basis based on Financial Support") +
         geom_bar(stat = "identity") + 
         ggtitle("Financial Support and Usage of Childcare") +
         scale_y_continuous(name="Count") + 
         scale_x_discrete(name="Financial Support") + 
    theme_bw()+
  theme(
...
```{r ccmainact, fig.cap="Main Activity by Respondents before/after parental leave", fig.pos="H", echo = FALSE }
...
theme_bw <- 
  ...
  ggplot(data = subset(data,!is.na(main_activity)),  aes(x = main_activity, y = frequency(main_activity), main="Child-Care Usage", fill = main_activity)) + labs(fill= "Main Activity") +
         geom_bar(stat = "identity") + 
         coord_flip() + 
         ggtitle("Main Activity Before/after Parental Leave") +
         scale_y_continuous(name="Count") +
...
```{r ccreason, fig.cap="Reasons to Select ChildCare Services", fig.pos="H", echo = FALSE}
...
theme_bw <- 
  ...
  ggplot(data = subset(data,!is.na(cc_reason)),  aes(x = cc_reason, y = frequency(cc_reason), main="Reason for Child-Care", fill = cc_reason)) + labs(fill= "Reason for Childcare") +
         geom_bar(stat = "identity") + 
         ggtitle("Reasons to Select ChildCare Services") +
         scale_y_continuous(name="Count") + 
...
```{r cctype, fig.cap="Child-Care Types Amongst Respondents",fig.pos="H", echo = FALSE}


theme_bw <- 
  options(repr.plot.width=15, repr.plot.height=6)
  ggplot(data = subset(data,!is.na(cc_type)),  aes(x = cc_type, y = frequency(cc_type), main="Child-Care Usage", fill = cc_type)) + labs(fill= "Childcare Type") +
         geom_bar(stat = "identity") + 
         coord_flip() + 
         ggtitle("Child-Care Types Amongst Respondents") +
         scale_y_continuous(name="Count") +
...
theme_bw <- 
  ...
  ggplot(data = subset(data,(!is.na(ever_fin_sup)) & (!is.na(cc_reason))),  aes(x = ever_fin_sup, y = frequency(ever_fin_sup), main="Child-Care Usage", fill = cc_reason)) +labs(fill= "Childcare reason based on Financial Support") +
         geom_bar(stat = "identity") + 
         coord_flip() + 
         ggtitle("Financial Support and Reason to get childcare") +
...
```{r cavgdailycost, fig.cap="Average Costs of Childcare Arrangement (per day)", echo = FALSE}
...
theme_bw <-
  ...
  ggplot(data = subset(data,!is.na(c_avg_daily_cost)),  aes(x = c_avg_daily_cost, y = frequency(c_avg_daily_cost), main="Child-Care Usage", fill = c_avg_daily_cost)) + labs(fill = "Average daily cost") +
         geom_bar(stat = "identity") +
         coord_flip() +
         ggtitle("Main Activity by Respondents before/after parental leave") +
         scale_y_continuous(name="Count") +
         scale_x_discrete(name="Average costs of child care arrangement (per day)") + ggtitle("Average Costs of Childcare Arrangement (per day)") +
    theme_bw()


```

```{r ccavgwkcost, fig.cap="Average Costs of Child Care Arrangement (per week)" , echo = FALSE}
...
```{r ccavgmonthlycost, fig.cap="Average Costs of Childcare Arrangement (per month)" , echo = FALSE}
...
theme_bw <-
  ...
  ggplot(data = subset(data,!is.na(cc_avg_monthly_cost)),  aes(x = cc_avg_monthly_cost, y = frequency(cc_avg_monthly_cost), main="Child-Care Usage", fill = cc_avg_monthly_cost)) + labs(fill = "Average monthly cost") +
         ...
         coord_flip() +
         scale_y_continuous(name="Count") +
         scale_x_discrete(name="Average costs of child care arrangement (per month)") + ggtitle("Average Costs of Childcare Arrangement (per month)") +
    theme_bw()
...
```{r TypeofEducation, fig.cap="Having Children vs. Level of Education", echo = FALSE}
# Grouped
...
```{r, echo=FALSE, out.width="50%", fig.cap="QR code of the Supplementary Survey"}
knitr::include_graphics("surveyqr.jpg")
```



```{r, echo=FALSE, out.width="50%", fig.cap="Questions from the Supplementary Survey"}
knitr::include_graphics("s1.jpg")
knitr::include_graphics("s2.jpg")
knitr::include_graphics("s3.jpg")
knitr::include_graphics("s4.jpg")
```
